...
__export(index_exports, {
  ...
  agentMeta: () => agentMeta,
  agentsTxtRoutes: () => agentsTxtRoutes,
  apiKeyAuth: () => apiKeyAuth,
  discoveryRoutes: () => discoveryRoutes,
  getOAuth2Token: () => getOAuth2Token,
  llmsTxtRoutes: () => llmsTxtRoutes,
  mcpServer: () => mcpServer,
  notFoundHandler: () => notFoundHandler,
  oauth2Auth: () => oauth2Auth,
  rateLimits: () => rateLimits,
  requireScope: () => requireScope,
  robotsTxtRoutes: () => robotsTxtRoutes,
...
// src/agent-errors.ts
var import_core = require("@agent-layer/core");
function agentErrors() {
  return (err, c) => {
    const result = (0, import_core.buildErrorResponse)(
      err,
      c.req.header("accept"),
...
function notFoundHandler() {
  return (c) => {
    const result = (0, import_core.buildNotFoundResponse)(
      c.req.method,
      c.req.path,
      c.req.header("accept"),
      c.req.header("user-agent")
...
// src/rate-limits.ts
var import_core2 = require("@agent-layer/core");
function rateLimits(config) {
  const check = (0, import_core2.createRateLimiter)(config);
  return async function rateLimitMiddleware(c, next) {
    ...
    if (!result.allowed) {
      ...
      const envelope = (0, import_core2.rateLimitError)(retryAfter);
...
// src/llms-txt.ts
var import_core3 = require("@agent-layer/core");
function llmsTxtRoutes(config, routes = []) {
  const txt = (0, import_core3.generateLlmsTxt)(config);
  const fullTxt = (0, import_core3.generateLlmsFullTxt)(config, routes);
  return {
    /**
...
// src/discovery.ts
var import_core4 = require("@agent-layer/core");
function discoveryRoutes(config) {
  const manifest = (0, import_core4.generateAIManifest)(config);
  const jsonLd = (0, import_core4.generateJsonLd)(config);
  return {
    /**
...
// src/agent-auth.ts
var import_core5 = require("@agent-layer/core");
function agentAuth(config) {
  const discovery = (0, import_core5.buildOauthDiscoveryDocument)(config);
  return {
    oauthDiscovery(c) {
    ...
    requireAuth() {
      return async function requireAuthMiddleware(c, next) {
        const result = (0, import_core5.checkRequireAuth)(config, c.req.header("authorization"));
        if (result.pass) {
          await next();
          return;
        }
...
// src/analytics.ts
var import_core6 = require("@agent-layer/core");
function agentAnalytics(config = {}) {
  const instance = (0, import_core6.createAnalytics)(config);
...
// src/api-keys.ts
var import_core7 = require("@agent-layer/core");
function apiKeyAuth(config) {
  ...
  return async function apiKeyAuthMiddleware(c, next) {
    ...
    if (!rawKey) {
      const envelope = (0, import_core7.formatError)({
    ...
    const result = await (0, import_core7.validateApiKey)(config.store, rawKey);
    if (!result.valid) {
      const envelope = (0, import_core7.formatError)({
        code: result.error,
...
function requireScope(scope) {
  return async function requireScopeMiddleware(c, next) {
    ...
    if (!(0, import_core7.hasScope)(agentKey, scope)) {
...
// src/a2a.ts
var import_core8 = require("@agent-layer/core");
function a2aRoutes(config) {
  const card = (0, import_core8.generateAgentCard)(config);
  return {
    /**
...
// src/agents-txt.ts
var import_core9 = require("@agent-layer/core");
function agentsTxtRoutes(config) {
  const content = (0, import_core9.generateStandaloneAgentsTxt)(config);
  return {
    agentsTxt(c) {
    ...
    enforce: ((c, next) => {
      ...
      const userAgent = c.req.header("user-agent") ?? "";
      const allowed = (0, import_core9.isAgentAllowed)(config, userAgent, c.req.path);
      if (allowed === false) {
        return c.json(
          {
            error: {
...
// src/robots-txt.ts
var import_core10 = require("@agent-layer/core");
function robotsTxtRoutes(config = {}) {
  const content = (0, import_core10.generateRobotsTxt)(config);
  return {
    robotsTxt(c) {
      c.header("Content-Type", "text/plain; charset=utf-8");
...
var import_core11 = require("@agent-layer/core");
function securityHeaders(config = {}) {
  const headers = (0, import_core11.generateSecurityHeaders)(config);
...
// src/x402.ts
var import_core12 = require("@agent-layer/core");
var import_x402 = require("@agent-layer/core/x402");
function x402Payment(config) {
  return async function x402Middleware(c, next) {
    const paymentHeader = c.req.header(import_x402.HEADER_PAYMENT_SIGNATURE);
    const url = new URL(c.req.url).toString();
    const result = await (0, import_core12.handleX402)(c.req.method, c.req.path, url, paymentHeader, config);
...
var import_core13 = require("@agent-layer/core");
  ...
  return {
    requireIdentity() {
      return async function requireIdentityMiddleware(c, next) {
        ...
        const result = await (0, import_core13.handleRequireIdentity)(rawHeader, config, {
          method: c.req.method,
          path: c.req.path,
          headers
        });
    ...
    optionalIdentity() {
      return async function optionalIdentityMiddleware(c, next) {
        const rawHeader = c.req.header(headerName);
        const claims = await (0, import_core13.handleOptionalIdentity)(rawHeader, config);
...
var import_core14 = require("@agent-layer/core");
function mcpServer(config) {
  const serverInfo = (0, import_core14.generateServerInfo)(config);
  const autoTools = config.routes ? (0, import_core14.generateToolDefinitions)(config.routes) : [];
  ...
  const defaultToolCallHandler = async (toolName, args) => {
    ...
    if (!routeInfo) {
      const parsed = (0, import_core14.parseToolName)(toolName);
      return {
        content: [
          {
            type: "text",
  ...
  return {
    app() {
      ...
      app.post("/", async (c) => {
        ...
        const result = (0, import_core14.handleJsonRpc)(body, serverInfo, allTools, defaultToolCallHandler);
        if (result === null) {
          return c.text("", 202);
        }
...
var import_core15 = require("@agent-layer/core");
function unifiedDiscovery(config) {
  const aiManifest = (0, import_core15.generateUnifiedAIManifest)(config);
  const agentCardDoc = (0, import_core15.generateUnifiedAgentCard)(config);
  const agentsTxtDoc = (0, import_core15.generateAgentsTxt)(config);
  const llmsTxtDoc = (0, import_core15.generateUnifiedLlmsTxt)(config);
  const llmsFullTxtDoc = (0, import_core15.generateUnifiedLlmsFullTxt)(config);
  ...
  if ((0, import_core15.isFormatEnabled)(config.formats, "wellKnownAi")) {
    app.get("/.well-known/ai", (c) => wellKnownAi(c));
  }
  if ((0, import_core15.isFormatEnabled)(config.formats, "agentCard")) {
    app.get("/.well-known/agent.json", (c) => agentCard(c));
  }
  if ((0, import_core15.isFormatEnabled)(config.formats, "agentsTxt")) {
    app.get("/agents.txt", (c) => agentsTxt(c));
  }
  if ((0, import_core15.isFormatEnabled)(config.formats, "llmsTxt")) {
...
var import_core16 = require("@agent-layer/core");
function agUiStream(handler, options = {}) {
  return (c) => {
    for (const [key, value] of Object.entries(import_core16.AG_UI_HEADERS)) {
      c.header(key, value);
    }
    return (0, import_streaming.stream)(c, async (s) => {
      const body = await c.req.json().catch(() => ({}));
      const emitter = (0, import_core16.createAgUiEmitter)(
        (chunk) => {
          s.write(chunk);
        },
        {
...
// src/oauth2.ts
var import_core17 = require("@agent-layer/core");
var OAUTH2_TOKEN_KEY = "oauth2Token";
function oauth2Auth(config) {
  const metadataDoc = (0, import_core17.buildOAuth2Metadata)(config);
  return {
    requireToken(requiredScopes) {
      ...
      return async function oauth2Middleware(c, next) {
        const result = await (0, import_core17.handleOAuth2)(c.req.header("authorization"), mwConfig);
        if (result.pass) {
